...
def apply_nms_fast(score_map, dist_thresh, conf_thresh=0.015):
    ...
    for idx in range(out.shape[1]):
        ...
        output_score_map[pts_nms[1, idx].astype(int), pts_nms[0, idx].astype(int)] = pts_nms[2, idx]#todo debug
...
def get_nms_score_map_from_score_map(
    ...
    对热力图进行非极大值抑制(NMS)处理
    Args:
        heatmap: 输入的热力图，形状为[H, W]
        conf_thresh: 置信度阈值，低于此值的点会被过滤掉
        nms_size: NMS窗口大小，在这个窗口内只保留最大值点
    Returns:
        nms_heatmap: 经过NMS处理后的热力图，形状与输入相同[H, W]
    """
    # 获取热力图的高度和宽度
    ...
    if len(xs) == 0:
        return np.zeros_like(heatmap)  # 如果没有点超过阈值，返回全零图
    
    # 创建点数据数组，形状为[3, N]
    # 第0行存储x坐标，第1行存储y坐标，第2行存储对应的置信度值
    ...
    # 对点进行NMS处理，抑制非极大值点
    pts, _ = nms_fast(pts, H, W, dist_thresh=nms_size)
    ...
    return nms_heatmap  # 返回处理后的热力图[H, W]
...
def nms_fast(in_corners, H, W, dist_thresh):
    ...
    快速非极大值抑制(NMS)实现
    Args:
        in_corners: 形状为[3, N]的数组，存储特征点信息
                   第0行: x坐标
                   第1行: y坐标
                   第2行: 置信度值
        H, W: 图像的高度和宽度
        dist_thresh: NMS抑制窗口的大小
    Returns:
        out: 经过NMS后保留的特征点信息[3, M]
        out_inds: 保留点在原始数组中的索引
    """
    # 创建网格用于追踪NMS状态: 0-未处理, 1-待处理, -1-保留点
    grid = np.zeros((H, W)).astype(int)  
    # 存储每个位置点在原始数组中的索引
    inds = np.zeros((H, W)).astype(int)  

    # 按置信度降序排序
    inds1 = np.argsort(-in_corners[2, :])  # 负号使得从大到小排序
    corners = in_corners[:, inds1]  # 重排特征点数组
    # 只取坐标信息(前两行)并四舍五入到整数
    rcorners = corners[:2, :].round().astype(int)

    # 处理边界情况
    if rcorners.shape[1] == 0:  # 没有特征点
        return np.zeros((3, 0)).astype(int), np.zeros(0).astype(int)
    if rcorners.shape[1] == 1:  # 只有一个特征点
        out = np.vstack((rcorners, in_corners[2])).reshape(3, 1)
        return out, np.zeros((1)).astype(int)
    ...
    # 主NMS循环：按置信度从高到低处理每个点
    count = 0
    for i, rc in enumerate(rcorners.T):
        # 考虑padding后的坐标
        pt = (rc[0] + pad, rc[1] + pad)
        if grid[pt[1], pt[0]] == 1:  # 如果该点还未被抑制
            # 将该点周围dist_thresh×dist_thresh窗口内的所有点置0(抑制)
            grid[pt[1] - pad:pt[1] + pad + 1, pt[0] - pad:pt[0] + pad + 1] = 0
            # 将当前点标记为保留点(-1)
            grid[pt[1], pt[0]] = -1
            count += 1
